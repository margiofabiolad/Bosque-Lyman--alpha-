...
\title{Bosque Lyman-$\alpha$}
kkkkkk
\lesoustitre{Le sous titre est facultatif}

\discipline{inscrire ici la section disciplinaire}

\dirdethese{Dr. Octavio Valenzuela}
\titredudirdethese{Universidad Nacional Autónoma de México (UNAM)}
...
\author{Margionet Fabiola Díaz}

\date{25 novembre 2004}

\nomdeuniversite{Universidad de Los Andes}
\logouniversite{ula} %nom du fichier
\scalelogouniversite{0.5} % mise a l'echelle de l'image (0.5=50% par exemple)
\logolabo{logo_labo}
\scalelogolabo{0.2}

\unite{Facultad de Ciencias}
\ecoledoc{Posgrado en Física Fundamental}
...
\section*{Transferendia Radiativa}
\addcontentsline{toc}{section}{Transferendia Radiativa}
